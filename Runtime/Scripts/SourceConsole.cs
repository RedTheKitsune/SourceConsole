...
namespace SourceConsole
{
    ...
    public static class SourceConsole
    {
        ...
        private static readonly List<string> CommandNamesCache = new List<string>();
        ...
        public static bool ShowFullErrorStack { get; set; }
        ...
        //This filters most unity and system assemblies
        private static Assembly[] GetNonUnityNonSystemAssemblies()
        {
            return AppDomain.CurrentDomain.GetAssemblies()
                .Where(a =>
                {
                    ...
                    // Exclusions (tune this list)
                    if (n.Contains("Unity", StringComparison.Ordinal)) return false;
                    if (n.Contains("System", StringComparison.Ordinal)) return false;
                    if (n.Contains("mscorlib", StringComparison.Ordinal)) return false;
                    if (n.Contains("netstandard", StringComparison.Ordinal)) return false;
                    if (n.Contains("Microsoft", StringComparison.Ordinal)) return false;
                    if (n.Contains("Mono.", StringComparison.Ordinal)) return false;
        ...
        public static int RefreshCommands(Assembly[] assemblies)
        {
            ...
            foreach (var command in SourceConsoleHelper.FindMethodAttributes<ConCommand>(assemblies))
            {
                ...
                {
                    MainThreadDispatcher.Post(() => { error($"Failed to load command '{name}'! A command/convar already exists with that name!"); });
            ...
            foreach (var convar in SourceConsoleHelper.FindPropertyAttributes<ConVar>(assemblies))
            {
                ...
                if (CommandNamesCache.Contains(name))
                {
                    MainThreadDispatcher.Post(() => { error($"Failed to load convar '{name}'! A command/convar already exists with that name!"); });
                ...
                if (convar.DefaultValue != null)
                {
                    ...
                    catch (Exception e)
                    {
                        if (ShowFullErrorStack)
                            MainThreadDispatcher.Post(() => { error(e); });
                        else
                            MainThreadDispatcher.Post(() => { error($"{e.GetType().Name}: {e.Message}"); });
                    }
                }
